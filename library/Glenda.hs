...
module Glenda (
  ...
  -- information and other documents, see <https://golange.org golang.org>.
  --
  ...
  -- * #Source_code_representation# Source code representation
  -- | Source code is Unicode text encoded in
  -- <https://en.wikipedia.org/wiki/UTF-8 UTF-8>. The text is not
  ...
  --
  -- A comment cannot start inside a <#Rune_literals rune> or
  ...
  --     -   an <#Identifiers identifier>
  --     -   an <#Integer_literals integer>,
  --         <#Floating-point_literals floating-point>,
  --         <#Imaginary_literals imaginary>, <#Rune_literals rune>, or
  --         <#String_literals string> literal
  --     -   one of the <#Keywords keywords> @break@, @continue@,
  --         @fallthrough@, or @return@
  --     -   one of the
  --         <#Operators_and_punctuation operators and punctuation> @++@,
  --         @--@, @)@, @]@, or @}@
  --
  ...
  -- (including <#assign_op assignment operators>) and punctuation:
  --
  -- > +    &     +=    &=     &&    ==    !=    (    )
  -- > -    |     -=    |=     ||    <     <=    [    ]
  -- > *    ^     *=    ^=     <-    >     >=    {    }
  ...
  -- ** #Integer_literals# Integer literals
  -- | An integer literal is a sequence of digits representing an
  -- <#Constants integer constant>. An optional prefix sets a non-decimal
  -- base: @0@ for octal, @0x@ or @0X@ for hexadecimal. In hexadecimal
  ...
  -- <#Constants floating-point constant>. It has an integer part, a decimal
  ...
  -- of a <#Constants complex constant>. It consists of a
  -- <#Floating-point_literals floating-point literal> or decimal integer
  ...
  -- identifying a Unicode code point. A rune literal is expressed as one or
  -- more characters enclosed in single quotes, as in @\'x\'@ or @\'\\n\'@.
  ...
  -- constant: @\\x@ followed by exactly two hexadecimal digits; @\\u@
  -- followed by exactly four hexadecimal digits; @\\U@ followed by exactly
  -- eight hexadecimal digits, and a plain backslash @\\@ followed by exactly
  -- three octal digits. In each case the value of the literal is the value
  ...
  -- construction. The escapes @\\u@ and @\\U@ represent Unicode code points
  -- so within them some values are illegal, in particular those above
  ...
  -- concatenating a sequence of characters. There are two forms: raw string
  ...
  -- they are in <#Rune_literals rune literals> (except that @\\\'@ is
  -- illegal and @\\\"@ is legal), with the same restrictions. The
  -- three-digit octal (@\\@/nnn/) and two-digit hexadecimal (@\\x@/nn/)
  ...
  -- /characters/. Thus inside a string literal @\\377@ and @\\xFF@ represent
  -- a single byte of value @0xFF@=255, while @ÿ@, @\\u00FF@, @\\U000000FF@
  -- and @\\xc3\\xbf@ represent the two bytes @0xc3@ @0xbf@ of the UTF-8
  ...
  --
  -- A constant value is represented by a <#Rune_literals rune>,
  -- <#Integer_literals integer>, <#Floating-point_literals floating-point>,
  -- <#Imaginary_literals imaginary>, or <#String_literals string> literal,
  -- an identifier denoting a constant, a
  -- <#Constant_expressions constant expression>, a <#Conversions conversion>
  -- with a result that is a constant, or the result value of some built-in
  -- functions such as @unsafe.Sizeof@ applied to any value, @cap@ or @len@
  -- applied to <#Length_and_capacity some expressions>, @real@ and @imag@
  ...
  -- <#Constant_expressions constant expression> and are discussed in that
  -- section.
  --
  ...
  --
  -- Constants may be <#Types typed> or /untyped/. Literal constants, @true@,
  -- @false@, @iota@, and certain
  -- <#Constant_expressions constant expressions> containing only untyped
  -- constant operands are untyped.
  --
  -- A constant may be given a type explicitly by a
  -- <#Constant_declarations constant declaration> or
  -- <#Conversions conversion>, or implicitly when used in a
  -- <#Variable_declarations variable declaration> or an
  -- <#Assignments assignment> or as an operand in an
  -- <#Expressions expression>. It is an error if the constant value cannot
  -- be <#Representability represented> as a value of the respective type.
  --
  ...
  -- required, for instance, in a
  -- <#Short_variable_declarations short variable declaration> such as
  -- @i := 0@ where there is no explicit type. The default type of an untyped
  ...
  -- evaluating <#Constant_expressions constant expressions>.

  -- * #Variables# Variables
  -- | A variable is a storage location for holding a /value/. The set of
  -- permissible values is determined by the variable\'s /<#Types type>/.
  --
  -- A <#Variable_declarations variable declaration> or, for function
  -- parameters and results, the signature of a
  -- <#Function_declarations function declaration> or
  -- <#Function_literals function literal> reserves storage for a named
  -- variable. Calling the built-in function <#Allocation new> or taking the
  -- address of a <#Composite_literals composite literal> allocates storage
  -- for a variable at run time. Such an anonymous variable is referred to
  -- via a (possibly implicit) <#Address_operators pointer indirection>.
  --
  -- /Structured/ variables of <#Array_types array>, <#Slice_types slice>,
  -- and <#Struct_types struct> types have elements and fields that may be
  -- <#Address_operators addressed> individually. Each such element acts like
  -- a variable.
  --
  ...
  -- variables are always <#Assignability assignable> to the static type of
  -- the variable.
  --
  ...
  -- <#Expressions expression>; it is the most recent value
  -- <#Assignments assigned> to the variable. If a variable has not yet been
  -- assigned a value, its value is the <#The_zero_value zero value> for its
  -- type.
  ...
  --
  -- The language <#Predeclared_identifiers predeclares> certain type names.
  -- Others are introduced with <#Type_declarations type declarations>.
  ...
  -- underlying type of the type to which @T@ refers in its
  ...
  -- <#Interface_types interface type> is its interface. The method set of
  -- any other type @T@ consists of all <#Method_declarations methods>
  -- declared with receiver type @T@. The method set of the corresponding
  -- <#Pointer_types pointer type> @*T@ is the set of all methods declared
  ...
  -- described in the section on <#Struct_types struct types>. Any other type
  -- has an empty method set. In a method set, each method must have a
  -- <#Uniqueness_of_identifiers unique> non-<#Blank_identifier blank>
  -- <#MethodName method name>.
  --
  -- The method set of a type determines the interfaces that the type
  -- <#Interface_types implements> and the methods that can be
  -- <#Calls called> using a receiver of that type.

  -- ** #Boolean_types# Boolean types
  ...
  -- type is @bool@; it is a <#Type_definitions defined type>.
  ...
  -- <https://en.wikipedia.org/wiki/Two's_complement two\'s complement arithmetic>.
  --
  ...
  -- <#Type_definitions defined types> and thus distinct except @byte@, which
  -- is an <#Alias_declarations alias> for @uint8@, and @rune@, which is an
  -- alias for @int32@. Conversions are required when different numeric types
  -- are mixed in an expression or assignment. For instance, @int32@ and
  -- @int@ are not the same type even though they may have the same size on a
  ...
  -- string type is @string@; it is a <#Type_definitions defined type>.
  --
  -- The length of a string @s@ (its size in bytes) can be discovered using
  -- the built-in function <#Length_and_capacity len>. The length is a
  -- compile-time constant if the string is a constant. A string\'s bytes can
  -- be accessed by integer <#Index_expressions indices> 0 through
  ...
  -- non-negative <#Constants constant> <#Representability representable> by
  -- a value of type @int@. The length of array @a@ can be discovered using
  -- the built-in function <#Length_and_capacity len>. The elements can be
  -- addressed by integer <#Index_expressions indices> 0 through @len(a)-1@.
  ...
  -- <#Length_and_capacity len>; unlike with arrays it may change during
  -- execution. The elements can be addressed by integer
  -- <#Index_expressions indices> 0 through @len(s)-1@. The slice index of a
  ...
  -- <#Slice_expressions slicing> a new one from the original slice. The
  ...
  -- using the built-in function <#Making_slices_maps_and_channels make>,
  ...
  -- <#Slice_expressions slicing> it, so these two expressions are
  -- equivalent:
  --
  -- > make([]int, 50, 100)
  -- > new([100]int)[0:50]
  ...
  -- non-<#Blank_identifier blank> field names must be
  -- <#Uniqueness_of_identifiers unique>.
  --
  ...
  -- or as a pointer to a non-interface type name @*T@, and @T@ itself may
  ...
  -- > }
  --
  -- A field or <#Method_declarations method> @f@ of an embedded field in a
  -- struct @x@ is called /promoted/ if @x.f@ is a legal
  -- <#Selectors selector> that denotes that field or method @f@.
  --
  -- Promoted fields act like ordinary fields of a struct except that they
  -- cannot be used as field names in
  -- <#Composite_literals composite literals> of the struct.
  --
  -- Given a struct type @S@ and a <#Type_definitions defined type> @T@,
  -- promoted methods are included in the method set of the struct as
  -- follows:
  --
  -- -   If @S@ contains an embedded field @T@, the
  --     <#Method_sets method sets> of @S@ and @*S@ both include promoted
  --     methods with receiver @T@. The method set of @*S@ also includes
  ...
  -- tags are made visible through a
  -- <https://golang.org/pkg/reflect/#StructTag reflection interface> and take
  -- part in <#Type_identity type identity> for structs but are otherwise
  -- ignored.
  --
  -- > struct {
  ...
  -- of a given type, called the /base type/ of the pointer. The value of an
  ...
  -- non-<#Blank_identifier blank> names in the signature must be
  -- <#Uniqueness_of_identifiers unique>. If absent, each type stands for one
  -- item of that type. Parameter and result lists are always parenthesized
  -- except that if there is exactly one unnamed result it may be written as
  ...
  -- <#Uniqueness_of_identifiers unique> non-<#Blank_identifier blank> name.
  --
  ...
  -- <#Type_declarations type declaration> to define an interface called
  -- @Locker@:
  --
  ...
  -- > KeyType     = Type .
  --
  -- The <#Comparison_operators comparison operators> @==@ and @!=@ must be
  ...
  -- failure will cause a <#Run_time_panics run-time panic>.
  --
  -- > map[string]int
  ...
  -- be discovered using the built-in function <#Length_and_capacity len> and
  -- may change during execution. Elements may be added during execution
  -- using <#Assignments assignments> and retrieved with
  -- <#Index_expressions index expressions>; they may be removed with the
  -- <#Deletion_of_map_elements delete> built-in function.
  --
  -- A new, empty map value is made using the built-in function
  -- <#Making_slices_maps_and_channels make>, which takes the map type and an
  ...
  -- <#Go_statements concurrently executing functions> to communicate by
  -- <#Send_statements sending> and <#Receive_operator receiving> values of a
  -- specified element type. The value of an uninitialized channel is @nil@.
  --
  ...
  -- <#Conversions conversion> or <#Assignments assignment>.
  --
  ...
  -- <#Making_slices_maps_and_channels make>, which takes the channel type
  ...
  -- multi-valued assignment form of the <#Receive_operator receive operator>
  -- reports whether a received value was sent before the channel was closed.
  --
  -- A single channel may be used in <#Send_statements send statements>,
  -- <#Receive_operator receive operations>, and calls to the built-in
  -- functions <#Length_and_capacity cap> and <#Length_and_capacity len> by
  -- any number of goroutines without further synchronization. Channels act
  ...
  --
  -- A <#Type_definitions defined type> is always different from any other
  -- type. Otherwise, two types are identical if their <#Types underlying>
  -- type literals are structurally equivalent; that is, they have the same
  ...
  --     <#Exported_identifiers Non-exported> field names from different
  ...
  --     <#Exported_identifiers Non-exported> method names from different
  ...
  --
  -- @B0@ and @B1@ are different because they are new types created by
  -- distinct <#Type_definitions type definitions>; @func(int, float64) *B0@
  -- and @func(x int, y float64) *[]string@ are different because @B0@ is
  -- different from @[]string@.

  -- ** #Assignability# Assignability
  -- | A value @x@ is /assignable/ to a <#Variables variable> of type @T@
  -- (\"@x@ is assignable to @T@\") if one of the following conditions
  -- applies:
  --
  -- -   @x@\'s type is identical to @T@.
  -- -   @x@\'s type @V@ and @T@ have identical <#Types underlying types> and
  --     at least one of @V@ or @T@ is not a <#Type_definitions defined>
  --     type.
  -- -   @T@ is an interface type and @x@ <#Interface_types implements> @T@.
  ...
  -- -   @x@ is an untyped <#Constants constant>
  --     <#Representability representable> by a value of type @T@.

  -- ** #Representability# Representability
  -- | A <#Constants constant> @x@ is /representable/ by a value of type @T@ if
  -- one of the following conditions applies:
  --
  -- -   @x@ is in the set of values <#Types determined> by @T@.
  ...
  -- -   @T@ is a complex type, and @x@\'s <#Complex_numbers components>
  ...
  -- 2.  Each <#Packages package> has a /package block/ containing all Go
  --     source text for that package.
  -- 3.  Each file has a /file block/ containing all Go source text in that
  --     file.
  -- 4.  Each <#If_statements \"if\">, <#For_statements \"for\">, and
  --     <#Switch_statements \"switch\"> statement is considered to be in its
  --     own implicit block.
  -- 5.  Each clause in a <#Switch_statements \"switch\"> or
  ...
  -- <#Constant_declarations constant>, <#Type_declarations type>,
  -- <#Variable_declarations variable>, <#Function_declarations function>,
  -- <#Labeled_statements label>, or <#Import_declarations package>. Every
  ...
  --
  -- The <#Blank_identifier blank identifier> may be used like any other
  ...
  -- only be used for <#Package_initialization init function> declarations,
  ...
  --
  -- 1.  The scope of a <#Predeclared_identifiers predeclared identifier> is
  --     the universe block.
  ...
  --
  -- The <#Package_clause package clause> is not a declaration; the package
  ...
  -- used in the <#Break_statements \"break\">,
  -- <#Continue_statements \"continue\">, and <#Goto_statements \"goto\">
  ...
  -- <#Declarations_and_scope declarations>, as an <#Operands operand>, and
  -- in <#Assignments assignments>.

  -- ** #Predeclared_identifiers# Predeclared identifiers
  -- | The following identifiers are implicitly declared in the
  -- <#Blocks universe block>:
  --
  -- > Types:
  ...
  -- ** #Exported_identifiers# Exported identifiers
  ...
  -- 2.  the identifier is declared in the <#Blocks package block> or it is a
  --     <#Struct_types field name> or <#MethodName method name>.
  --
  -- All other identifiers are not exported.

  -- ** #Uniqueness_of_identifiers# Uniqueness of identifiers
  ...
  -- <#Packages packages> and are not <#Exported_identifiers exported>.
  ...
  -- <#Constant_expressions constant expressions>. The number of identifiers
  -- must be equal to the number of expressions, and the /n/th identifier on
  ...
  -- expressions must be <#Assignability assignable> to that type. If the
  ...
  -- be omitted from any but the first ConstSpec. Such an empty list is
  ...
  -- list. Together with the <#Iota iota constant generator> this mechanism
  -- permits light-weight declaration of sequential values:
  --
  -- > const (
  -- >     Sunday = iota
  -- >     Monday
  ...
  -- <#Constant_declarations implicit repetition> of the last non-empty
  -- expression list.

  -- ** #Type_declarations# Type declarations
  -- | A type declaration binds an identifier, the /type name/, to a
  -- <#Types type>. Type declarations come in two forms: alias declarations
  ...
  --
  -- Within the <#Declarations_and_scope scope> of the identifier, it serves
  ...
  -- <#Types underlying type> and operations as the given type, and binds an
  -- identifier to it.
  --
  -- > TypeDef = identifier Type .
  --
  -- The new type is called a /defined type/. It is
  -- <#Type_identity different> from any other type, including the type it is
  -- created from.
  --
  -- > type (
  ...
  -- > }
  --
  -- A defined type may have <#Method_declarations methods> associated with
  -- it. It does not inherit any methods bound to the given type, but the
  -- <#Method_sets method set> of an interface type or of elements of a
  ...
  -- <#Conversions converted> to its <#Constants default type>; if it is an
  ...
  -- <#Variable_declarations variable declaration> with initializer
  ...
  -- non-<#Blank_identifier blank> variables is new. As a consequence,
  ...
  -- > FunctionBody = Block .
  --
  -- If the function\'s <#Function_types signature> declares result
  ...
  -- interface type and it must be <#Type_definitions defined> in the same
  ...
  -- type @T@ or @*T@.
  --
  -- A non-<#Blank_identifier blank> receiver identifier must be
  -- <#Uniqueness_of_identifiers unique> in the method signature. If the
  ...
  -- unique. If the base type is a <#Struct_types struct type>, the non-blank
  -- method and field names must be distinct.
  --
  ...
  -- be a literal, a (possibly <#Qualified_identifiers qualified>)
  -- non-<#Blank_identifier blank> identifier denoting a
  -- <#Constant_declarations constant>, <#Variable_declarations variable>, or
  -- <#Function_declarations function>, or a parenthesized expression.
  --
  -- The <#Blank_identifier blank identifier> may appear as an operand only
  -- on the left-hand side of an <#Assignments assignment>.
  --
  ...
  -- which must be <#Import_declarations imported>. The identifier must be
  -- <#Exported_identifiers exported> and declared in the
  ...
  --
  -- The LiteralType\'s underlying type must be a struct, array, slice, or
  -- map type (the grammar enforces this constraint except when the type is
  -- given as a TypeName). The types of the elements and keys must be
  -- <#Assignability assignable> to the respective field, element, and key
  ...
  -- keys, see the section on <#Order_of_evaluation evaluation order>.
  --
  ...
  --     non-negative constant <#Representability representable> by a value
  --     of type @int@; and if it is typed it must be of integer type.
  -- -   An element without a key uses the previous element\'s index plus
  --     one. If the first element has no key, its index is zero.
  --
  -- <#Address_operators Taking the address> of a composite literal generates
  -- a pointer to a unique <#Variables variable> initialized with the
  -- literal\'s value.
  --
  ...
  -- <#Keywords keyword> and the opening brace of the block of an \"if\",
  -- \"for\", or \"switch\" statement, and the composite literal is not
  ...
  -- <#Package_clause package name>, the /selector expression/
  --
  -- > x.f
  --
  ...
  -- it must not be the <#Blank_identifier blank identifier>. The type of the
  -- selector expression is the type of @f@. If @x@ is a package name, see
  -- the section on <#Qualified_identifiers qualified identifiers>.
  --
  -- A selector @f@ may denote a field or method @f@ of a type @T@, or it may
  -- refer to a field or method @f@ of a nested
  -- <#Struct_types embedded field> of @T@. The number of embedded fields
  -- traversed to reach @f@ is called its /depth/ in @T@. The depth of a
  -- field or method @f@ declared in @T@ is zero. The depth of a field or
  -- method @f@ declared in an embedded field @A@ in @T@ is the depth of @f@
  -- in @A@ plus one.
  --
  ...
  --     <#Uniqueness_of_identifiers one f> with shallowest depth, the
  ...
  --     If there is no method with name @f@ in the <#Method_sets method set>
  --     of @I@, the selector expression is illegal.
  -- 3.  As an exception, if the type of @x@ is a <#Type_definitions defined>
  --     pointer type and @(*x).f@ is a valid selector expression denoting a
  ...
  --     a struct field, assigning to or evaluating @x.f@ causes a
  --     <#Run_time_panics run-time panic>.
  -- 6.  If @x@ is of interface type and has the value @nil@,
  --     <#Calls calling> or <#Method_values evaluating> the method @x.f@
  --     causes a <#Run_time_panics run-time panic>.
  --
  ...
  -- | If @M@ is in the <#Method_sets method set> of type @T@, @T.M@ is a
  ...
  -- construct a function that binds the receiver, use a
  -- <#Function_literals function literal> or <#Method_values method value>.
  --
  ...
  -- <#Method_sets method set> of type @T@, @x.M@ is called a /method value/.
  -- The method value @x.M@ is a function value that is callable with the
  -- same arguments as a method call of @x.M@. The expression @x@ is
  -- evaluated and saved during the evaluation of the method value; the saved
  -- copy is then used as the receiver in any calls, which may be executed
  -- later.
  --
  -- The type @T@ may be an interface or non-interface type.
  --
  -- As in the discussion of <#Method_expressions method expressions> above,
  -- consider a struct type @T@ with two methods, @Mv@, whose receiver is of
  -- type @T@, and @Mp@, whose receiver is of type @*T@.
  --
  -- > type T struct {
  ...
  --
  -- As with <#Calls method calls>, a reference to a non-interface method
  ...
  -- ** #Index_expressions# Index expressions
  ...
  --     <#Representability representable> by a value of type @int@
  -- -   a constant index that is untyped is given type @int@
  -- -   the index @x@ is /in range/ if @0 \<= x \< len(a)@, otherwise it is
  --     /out of range/
  --
  -- For @a@ of <#Array_types array type> @A@:
  --
  -- -   a <#Constants constant> index must be in range
  -- -   if @x@ is out of range at run time, a
  --     <#Run_time_panics run-time panic> occurs
  -- -   @a[x]@ is the array element at index @x@ and the type of @a[x]@ is
  --     the element type of @A@
  --
  -- For @a@ of <#Pointer_types pointer> to array type:
  --
  -- -   @a[x]@ is shorthand for @(*a)[x]@
  --
  -- For @a@ of <#Slice_types slice type> @S@:
  --
  -- -   if @x@ is out of range at run time, a
  --     <#Run_time_panics run-time panic> occurs
  ...
  --
  -- For @a@ of <#String_types string type>:
  --
  -- -   a <#Constants constant> index must be in range if the string @a@ is
  --     also constant
  -- -   if @x@ is out of range at run time, a
  --     <#Run_time_panics run-time panic> occurs
  ...
  --
  -- For @a@ of <#Map_types map type> @M@:
  --
  -- -   @x@\'s type must be <#Assignability assignable> to the key type of
  --     @M@
  ...
  --     <#The_zero_value zero value> for the element type of @M@
  --
  -- Otherwise @a[x]@ is illegal.
  --
  -- An index expression on a map @a@ of type @map[K]V@ used in an
  -- <#Assignments assignment> or initialization of the special form
  --
  -- > v, ok = a[x]
  -- > v, ok := a[x]
  -- > var v, ok = a[x]
  ...
  -- ** #Slice_expressions# Slice expressions
  ...
  -- A <#Constants constant> index must be non-negative and
  -- <#Representability representable> by a value of type @int@; for arrays
  ...
  -- are out of range at run time, a <#Run_time_panics run-time panic>
  -- occurs.
  --
  -- Except for <#Constants untyped strings>, if the sliced operand is a
  ...
  -- is an array, it must be <#Address_operators addressable> and the result
  ...
  -- it controls the resulting slice\'s capacity by setting it to
  ...
  -- sliced operand is an array, it must be <#Address_operators addressable>.
  --
  -- The indices are /in range/ if @0 \<= low \<= high \<= max \<= cap(a)@,
  -- otherwise they are /out of range/. A <#Constants constant> index must be
  -- non-negative and <#Representability representable> by a value of type
  -- @int@; for arrays, constant indices must also be in range. If multiple
  -- indices are constant, the constants that are present must be in range
  -- relative to each other. If the indices are out of range at run time, a
  -- <#Run_time_panics run-time panic> occurs.

  -- ** #Type_assertions# Type assertions
  -- | For an expression @x@ of <#Interface_types interface type> and a type
  ...
  -- the dynamic type of @x@ is <#Type_identity identical> to the type @T@.
  -- In this case, @T@ must <#Method_sets implement> the (interface) type of
  ...
  -- <#Run_time_panics run-time panic> occurs. In other words, even though
  ...
  -- > }
  --
  -- A type assertion used in an <#Assignments assignment> or initialization
  -- of the special form
  --
  -- > v, ok = x.(T)
  -- > v, ok := x.(T)
  ...
  -- the <#The_zero_value zero value> for type @T@. No run-time panic occurs
  -- in this case.
  ...
  -- arguments must be single-valued expressions <#Assignability assignable>
  ...
  -- <#Order_of_evaluation the usual order>. After they are evaluated, the
  ...
  -- returns.
  --
  -- Calling a @nil@ function value causes a
  -- <#Run_time_panics run-time panic>.
  --
  ...
  -- > }
  --
  -- A method call @x.m()@ is valid if the <#Method_sets method set> of (the
  -- type of) @x@ contains @m@ and the argument list can be assigned to the
  -- parameter list of @m@. If @x@ is <#Address_operators addressable> and
  ...
  -- ** #Passing_arguments_to_..._parameters# Passing arguments to @...@ parameters
  -- | If @f@ is <#Function_types variadic> with a final parameter @p@ of type
  ...
  -- arguments, which all must be <#Assignability assignable> to @T@. The
  ...
  -- > unary_op   = "+" | "-" | "!" | "^" | "*" | "&" | "<-" .
  --
  -- Comparisons are discussed <#Comparison_operators elsewhere>. For other
  -- binary operators, the operand types must be <#Type_identity identical>
  ...
  -- <#Constants constant> and the other operand is not, the constant is
  -- <#Conversions converted> to the type of the other operand.
  --
  -- The right operand in a shift expression must have unsigned integer type
  -- or be an untyped constant <#Representability representable> by a value
  ...
  -- *** #Integer_operators# Integer operators
  -- | For two integer values @x@ and @y@, the integer quotient @q = x \/ y@
  -- and remainder @r = x % y@ satisfy the following relationships:
  --
  -- > x = q*y + r  and  |r| < |y|
  --
  -- with @x \/ y@ truncated towards zero
  -- (<https://en.wikipedia.org/wiki/Modulo_operation \"truncated division\">).
  --
  -- >  x     y     x / y     x % y
  -- >  5     3       1         2
  -- > -5     3      -1        -2
  ...
  -- equal to @x@ (and @r = 0@) due to two\'s-complement
  ...
  --
  -- If the divisor is a <#Constants constant>, it must not be zero. If the
  -- divisor is zero at run time, a <#Run_time_panics run-time panic> occurs.
  -- If the dividend is non-negative and the divisor is a constant power of
  -- 2, the division may be replaced by a right shift, and computing the
  ...
  -- *** #Integer_overflow# Integer overflow
  -- | For unsigned integer values, the operations @+@, @-@, @*@, and @\<\<@
  -- are computed modulo 2/n/, where /n/ is the bit width of the
  -- <#Numeric_types unsigned integer>\'s type. Loosely speaking, these
  ...
  -- *** #Floating_point_operators# Floating-point operators
  ...
  -- a <#Run_time_panics run-time panic> occurs is implementation-specific.
  --
  ...
  -- <#Conversions conversion> explicitly rounds to the precision of the
  ...
  -- > >=    greater or equal
  --
  -- In any comparison, the first operand must be <#Assignability assignable>
  ...
  --     distinct <#Size_and_alignment_guarantees zero-size> variables may or
  --     may not be equal.
  -- -   Channel values are comparable. Two channel values are equal if they
  --     were created by the same call to
  --     <#Making_slices_maps_and_channels make> or if both have value @nil@.
  -- -   Interface values are comparable. Two interface values are equal if
  --     they have <#Type_identity identical> dynamic types and equal dynamic
  ...
  --     non-<#Blank_identifier blank> fields are equal.
  ...
  -- a <#Run_time_panics run-time panic> if values of that type are not
  ...
  -- | Logical operators apply to <#Boolean_types boolean> values and yield a
  ...
  -- ** #Address_operators# Address operators
  ...
  -- requirement, @x@ may also be a (possibly parenthesized)
  -- <#Composite_literals composite literal>. If the evaluation of @x@ would
  -- cause a <#Run_time_panics run-time panic>, then the evaluation of @&x@
  -- does too.
  --
  -- For an operand @x@ of pointer type @*T@, the pointer indirection @*x@
  -- denotes the <#Variables variable> of type @T@ pointed to by @x@. If @x@
  -- is @nil@, an attempt to evaluate @*x@ will cause a
  -- <#Run_time_panics run-time panic>.
  --
  -- > &x
  -- > &a[f(2)]
  ...
  -- ** #Receive_operator# Receive operator
  -- | For an operand @ch@ of <#Channel_types channel type>, the value of the
  ...
  -- forever. A receive operation on a <#Close closed> channel can always
  -- proceed immediately, yielding the element type\'s
  -- <#The_zero_value zero value> after any previously sent values have been
  -- received.
  --
  -- > v1 := <-ch
  -- > v2 = <-ch
  ...
  --
  -- A receive expression used in an <#Assignments assignment> or
  ...
  --
  -- A <#Constants constant> value @x@ can be converted to type @T@ if @x@ is
  -- <#Representability representable> by a value of @T@. As a special case,
  -- an integer constant @x@ can be converted to a
  -- <#String_types string type> using the
  -- <#Conversions_to_and_from_a_string_type same rule> as for non-constant
  -- @x@.
  --
  ...
  -- cases:
  --
  -- -   @x@ is <#Assignability assignable> to @T@.
  -- -   ignoring struct tags (see below), @x@\'s type and @T@ have
  --     <#Type_identity identical> <#Types underlying types>.
  -- -   ignoring struct tags (see below), @x@\'s type and @T@ are pointer
  --     types that are not <#Type_definitions defined types>, and their
  ...
  --
  -- <#Struct_types Struct tags> are ignored when comparing struct types for
  ...
  -- *** #Conversions_to_and_from_a_string_type# Conversions to and from a string type
  ...
  --
  -- 2.  Converting a slice of bytes to a string type yields a string whose
  ...
  -- ** #Constant_expressions# Constant expressions
  -- | Constant expressions may contain only <#Constants constant> operands and
  ...
  --
  -- A constant <#Comparison_operators comparison> always yields an untyped
  -- boolean constant. If the left operand of a constant
  -- <#Operators shift expression> is an untyped constant, the result is an
  -- integer constant; otherwise it is a constant of the same type as the
  -- left operand, which must be of <#Numeric_types integer type>. Applying
  -- all other operators to untyped constants results in an untyped constant
  -- of the same kind (that is, a boolean, integer, floating-point, complex,
  -- or string constant).
  --
  ...
  -- <#Representability representable> by values of the constant type. The
  -- following constant expressions are illegal:
  --
  ...
  -- ** #Order_of_evaluation# Order of evaluation
  -- | At package level, <#Package_initialization initialization dependencies>
  -- determine the evaluation order of individual initialization expressions
  -- in <#Variable_declarations variable declarations>. Otherwise, when
  -- evaluating the <#Operands operands> of an expression, assignment, or
  -- <#Return_statements return statement>, all function calls, method calls,
  -- and communication operations are evaluated in lexical left-to-right
  -- order.
  --
  ...
  --
  -- At package level, initialization dependencies override the left-to-right
  -- rule for individual initialization expressions, but not for operands
  ...
  --
  -- The function calls happen in the order @u()@, @sqr()@, @v()@, @f()@,
  -- @v()@, and @g()@.
  --
  ...
  -- lexically appear after it in the same <#Blocks block>. The following
  -- statements are terminating:
  --
  -- 1.  A <#Return_statements \"return\"> or <#Goto_statements \"goto\">
  --     statement.
  --
  -- 2.  A call to the built-in function <#Handling_panics panic>.
  --
  -- 3.  A <#Blocks block> in which the statement list ends in a terminating
  --     statement.
  --
  -- 4.  An <#If_statements \"if\" statement> in which:
  --     -   the \"else\" branch is present, and
  --     -   both branches are terminating statements.
  --
  -- 5.  A <#For_statements \"for\" statement> in which:
  --     -   there are no \"break\" statements referring to the \"for\"
  --         statement, and
  --     -   the loop condition is absent.
  --
  -- 6.  A <#Switch_statements \"switch\" statement> in which:
  --     -   there are no \"break\" statements referring to the \"switch\"
  --         statement,
  --     -   there is a default case, and
  --     -   the statement lists in each case, including the default, end in
  --         a terminating statement, or a possibly labeled
  --         <#Fallthrough_statements \"fallthrough\" statement>.
  --
  -- 7.  A <#Select_statements \"select\" statement> in which:
  --     -   there are no \"break\" statements referring to the \"select\"
  --         statement, and
  --     -   the statement lists in each case, including the default if
  --         present, end in a terminating statement.
  --
  -- 8.  A <#Labeled_statements labeled statement> labeling a terminating
  --     statement.
  --
  -- All other statements are not terminating.
  --
  -- A <#Blocks statement list> ends in a terminating statement if the list
  ...
  -- <#Calls calls> and <#Receive_operator receive operations> can appear in
  ...
  -- be of <#Channel_types channel type>, the channel direction must permit
  -- send operations, and the type of the value to be sent must be
  -- <#Assignability assignable> to the channel\'s element type.
  --
  ...
  -- send on an unbuffered channel can proceed if a receiver is ready. A send
  -- on a buffered channel can proceed if there is room in the buffer. A send
  -- on a closed channel proceeds by causing a
  -- <#Run_time_panics run-time panic>. A send on a @nil@ channel blocks
  -- forever.
  --
  ...
  -- by the untyped <#Constants constant> @1@. As with an assignment, the
  -- operand must be <#Address_operators addressable> or a map index
  -- expression.
  --
  -- > IncDecStmt = Expression ( "++" | "--" ) .
  --
  -- The following <#Assignments assignment statements> are semantically
  -- equivalent:
  --
  ...
  --
  -- Each left-hand side operand must be <#Address_operators addressable>, a
  -- map index expression, or (for @=@ assignments only) the
  -- <#Blank_identifier blank identifier>. Operands may be parenthesized.
  --
  -- > x = 1
  -- > *p = f()
  -- > a[i] = 23
  -- > (k) = <-ch  // same as: k = <-ch
  --
  -- An /assignment operation/ @x@ /op/@=@ @y@ where /op/ is a binary
  -- <#Arithmetic_operators arithmetic operator> is equivalent to @x@ @=@ @x@
  -- /op/ @(y)@ but evaluates @x@ only once. The /op/@=@ construct is a
  -- single token. In assignment operations, both the left- and right-hand
  -- expression lists must contain exactly one single-valued expression, and
  ...
  -- function call, a <#Channel_types channel> or <#Map_types map> operation,
  -- or a <#Type_assertions type assertion>. The number of operands on the
  -- left hand side must match the number of values. For instance, if @f@ is
  -- a function returning two values,
  --
  -- > x, y = f()
  --
  -- assigns the first value to @x@ and the second to @y@. In the second
  -- form, the number of operands on the left must equal the number of
  -- expressions on the right, each of which must be single-valued, and the
  -- /n/th expression on the right is assigned to the /n/th operand on the
  -- left:
  --
  -- > one, two, three = '一', '二', '三'
  --
  -- The <#Blank_identifier blank identifier> provides a way to ignore
  ...
  -- <#Index_expressions index expressions> and
  -- <#Address_operators pointer indirections> (including implicit pointer
  -- indirections in <#Selectors selectors>) on the left and the expressions
  -- on the right are all
  -- <#Order_of_evaluation evaluated in the usual order>. Second, the
  ...
  --
  -- In assignments, each value must be <#Assignability assignable> to the
  -- type of the operand to which it is assigned, with the following special
  -- cases:
  --
  -- 1.  Any typed value may be assigned to the blank identifier.
  -- 2.  If an untyped constant is assigned to a variable of interface type
  --     or the blank identifier, the constant is first
  ...
  -- expression is equivalent to the boolean value @true@.
  --
  ...
  --
  -- If a case expression is untyped, it is first <#Conversions converted> to
  -- the type of the switch expression. For each (possibly converted) case
  -- expression @x@ and the value @t@ of the switch expression, @x == t@ must
  -- be a valid <#Comparison_operators comparison>.
  --
  -- In other words, the switch expression is treated as if it were used to
  -- declare and initialize a temporary variable @t@ without explicit type;
  ...
  -- (possibly <#Labeled_statements labeled>)
  -- <#Fallthrough_statements \"fallthrough\" statement> to indicate that
  -- control should flow from the end of this clause to the first statement
  -- of the next clause. Otherwise control flows to the end of the \"switch\"
  ...
  -- that has the form of a <#Type_assertions type assertion> using the
  -- reserved word @type@ rather than an actual type:
  --
  -- > switch x.(type) {
  -- > // cases
  -- > }
  ...
  -- <#Interface_types interface type>, and each non-interface type @T@
  -- listed in a case must implement the type of @x@. The types listed in the
  -- cases of a type switch must all be <#Type_identity different>.
  --
  ...
  -- <#Short_variable_declarations short variable declaration>. When that
  -- form is used, the variable is declared at the end of the TypeSwitchCase
  -- in the <#Blocks implicit block> of each clause. In clauses with a case
  ...
  -- <#Predeclared_identifiers nil>; that case is selected when the
  -- expression in the TypeSwitchGuard is a @nil@ interface value. There may
  -- be at most one @nil@ case.
  --
  -- Given an expression @x@ of type @interface{}@, the following type
  -- switch:
  --
  -- > switch i := x.(type) {
  -- > case nil:
  ...
  -- three forms: The iteration may be controlled by a single condition, a
  -- \"for\" clause, or a \"range\" clause.
  --
  ...
  -- *** #For_clause# For statements with @for@ clause
  -- | A \"for\" statement with a ForClause is also controlled by its
  -- condition, but additionally it may specify an /init/ and a /post/
  -- statement, such as an assignment, an increment or decrement statement.
  -- The init statement may be a
  -- <#Short_variable_declarations short variable declaration>, but the post
  -- statement must not. Variables declared by the init statement are re-used
  -- in each iteration.
  --
  ...
  -- element of the ForClause may be empty but the <#Semicolons semicolons>
  -- are required unless there is only a condition. If the condition is
  -- absent, it is equivalent to the boolean value @true@.
  --
  ...
  -- each entry it assigns /iteration values/ to corresponding /iteration
  ...
  -- expression/, which may be an array, pointer to an array, slice, string,
  -- map, or channel permitting <#Receive_operator receive operations>. As
  -- with an assignment, if present the operands on the left must be
  -- <#Address_operators addressable> or map index expressions; they denote
  -- the iteration variables. If the range expression is a channel, at most
  -- one iteration variable is permitted, otherwise there may be up to two.
  -- If the last iteration variable is the
  -- <#Blank_identifier blank identifier>, the range clause is equivalent to
  -- the same clause without that identifier.
  --
  -- The range expression @x@ is evaluated once before beginning the loop,
  -- with one exception: if at most one iteration variable is present and
  -- @len(x)@ is <#Length_and_capacity constant>, the range expression is not
  -- evaluated.
  --
  -- Function calls on the left are evaluated once per iteration. For each
  -- iteration, iteration values are produced as follows if the respective
  -- iteration variables are present:
  --
  ...
  --
  -- 1.  For an array, pointer to array, or slice value @a@, the index
  --     iteration values are produced in increasing order, starting at
  --     element index 0. If at most one iteration variable is present, the
  --     range loop produces iteration values from 0 up to @len(a)-1@ and
  --     does not index into the array or slice itself. For a @nil@ slice,
  --     the number of iterations is 0.
  -- 2.  For a string value, the \"range\" clause iterates over the Unicode
  ...
  --     second value will be @0xFFFD@, the Unicode replacement character,
  ...
  --     values sent on the channel until the channel is <#Close closed>. If
  ...
  -- as in an <#Assignments assignment statement>.
  --
  -- The iteration variables may be declared by the \"range\" clause using a
  -- form of <#Short_variable_declarations short variable declaration>
  -- (@:=@). In this case their types are set to the types of the respective
  -- iteration values and their <#Declarations_and_scope scope> is the block
  -- of the \"for\" statement; they are re-used in each iteration. If the
  -- iteration variables are declared outside the \"for\" statement, after
  ...
  -- <#Expression_statements expression statements>.
  --
  -- The function value and parameters are <#Calls evaluated as usual> in the
  ...
  -- function terminates, its goroutine also terminates. If the function has
  ...
  -- <#Send_statements send> or <#Receive_operator receive> operations will
  -- proceed. It looks similar to a <#Switch_statements \"switch\"> statement
  ...
  -- variables, which may be declared using a
  -- <#Short_variable_declarations short variable declaration>. The RecvExpr
  -- must be a (possibly parenthesized) receive operation. There can be at
  ...
  --     entering the \"select\" statement. The result is a set of channels
  --     to receive from or send to, and the corresponding values to send.
  --     Any side effects in that evaluation will occur irrespective of which
  --     (if any) communication operation is selected to proceed. Expressions
  --     on the left-hand side of a RecvStmt with a short variable
  ...
  --     is no default case, the \"select\" statement blocks until at least
  ...
  -- 4.  If the selected case is a RecvStmt with a short variable declaration
  --     or an assignment, the left-hand side expressions are evaluated and
  --     the received value (or values) are assigned.
  ...
  -- @F@, and optionally provides one or more result values. Any functions
  -- <#Defer_statements deferred> by @F@ are executed before @F@ returns to
  -- its caller.
  --
  ...
  -- type:
  --
  -- 1.  The return value or values may be explicitly listed in the
  --     \"return\" statement. Each expression must be single-valued and
  --     <#Assignability assignable> to the corresponding element of the
  ...
  --     > }
  --
  -- 2.  The expression list in the \"return\" statement may be a single call
  --     to a multi-valued function. The effect is as if each value returned
  --     from that function were assigned to a temporary variable with the
  --     type of the respective value, followed by a \"return\" statement
  --     listing these variables, at which point the rules of the previous
  --     case apply.
  --
  ...
  --     > }
  --
  -- 3.  The expression list may be empty if the function\'s result type
  --     specifies names for its <#Function_types result parameters>. The
  --     result parameters act as ordinary local variables and the function
  --     may assign values to them as necessary. The \"return\" statement
  ...
  -- initialized to the <#The_zero_value zero values> for their type upon
  -- entry to the function. A \"return\" statement that specifies results
  ...
  -- variable) with the same name as a result parameter is in
  -- <#Declarations_and_scope scope> at the place of the return.
  --
  ...
  -- <#For_statements \"for\">, <#Switch_statements \"switch\">, or
  -- <#Select_statements \"select\"> statement within the same function.
  --
  -- > BreakStmt = "break" [ Label ] .
  --
  -- If there is a label, it must be that of an enclosing \"for\",
  -- \"switch\", or \"select\" statement, and that is the one whose execution
  -- terminates.
  --
  -- > OuterLoop:
  -- >     for i = 0; i < n; i++ {
  ...
  -- <#For_statements \"for\" loop> at its post statement. The \"for\" loop
  ...
  -- corresponding label within the same function.
  --
  -- > GotoStmt = "goto" Label .
  --
  -- > goto Error
  ...
  -- into <#Declarations_and_scope scope> that were not already in scope at
  -- the point of the goto. For instance, this example:
  --
  -- >     goto L  // BAD
  -- >     v := 3
  -- > L:
  ...
  --
  -- A \"goto\" statement outside a <#Blocks block> cannot jump to a label
  -- inside that block. For instance, this example:
  --
  -- > if n%2 == 1 {
  -- >     goto L1
  -- > }
  ...
  -- the next case clause in an
  -- <#Expression_switches expression \"switch\" statement>. It may be used
  ...
  -- surrounding function executed a <#Return_statements return statement>,
  -- reached the end of its <#Function_declarations function body>, or
  -- because the corresponding goroutine is <#Handling_panics panicking>.
  --
  ...
  -- parameters to the call are <#Calls evaluated as usual> and saved anew
  -- but the actual function is not invoked. Instead, deferred functions are
  -- invoked immediately before the surrounding function returns, in the
  -- reverse order they were deferred. If a deferred function value evaluates
  -- to @nil@, execution <#Handling_panics panics> when the function is
  -- invoked, not when the \"defer\" statement is executed.
  --
  -- For instance, if the deferred function is a
  -- <#Function_literals function literal> and the surrounding function has
  -- <#Function_types named result parameters> that are in scope within the
  ...
  -- return values, they are discarded when the function completes. (See also
  -- the section on <#Handling_panics handling panics>.)
  --
  -- > lock(l)
  ...
  -- called like any other function but some of them accept a type instead of
  -- an expression as the first argument.
  --
  -- The built-in functions do not have standard Go types, so they can only
  -- appear in <#Calls call expressions>; they cannot be used as function
  -- values.
  ...
  -- <#Run_time_panics run-time panic>. Closing the nil channel also causes a
  -- <#Run_time_panics run-time panic>. After calling @close@, and after any
  -- previously sent values have been received, receive operations will
  -- return the zero value for the channel\'s type without blocking. The
  -- multi-valued <#Receive_operator receive operation> returns a received
  -- value along with an indication of whether the channel is closed.

  -- ** #Length_and_capacity# Length and capacity
  ...
  -- > 0 <= len(s) <= cap(s)
  --
  -- The length of a @nil@ slice, map or channel is 0. The capacity of a
  -- @nil@ slice or channel is 0.
  --
  -- The expression @len(s)@ is <#Constants constant> if @s@ is a string
  -- constant. The expressions @len(s)@ and @cap(s)@ are constants if the
  -- type of @s@ is an array or pointer to an array and the expression @s@
  -- does not contain <#Receive_operator channel receives> or (non-constant)
  -- <#Calls function calls>; in this case @s@ is not evaluated. Otherwise,
  -- invocations of @len@ and @cap@ are not constant and @s@ is evaluated.
  --
  -- > const (
  ...
  -- <#Variables variable> of that type at run time, and returns a value of
  -- type @*T@ <#Pointer_types pointing> to it. The variable is initialized
  -- as described in the section on <#The_zero_value initial values>.
  --
  -- > new(T)
  --
  -- For instance
  ...
  -- ** #Making_slices_maps_and_channels# Making slices, maps and channels
  ...
  -- initialized as described in the section on
  -- <#The_zero_value initial values>.
  --
  -- > Call             Type T     Result
  -- >
  ...
  --
  -- Each of the size arguments @n@ and @m@ must be of integer type or an
  -- untyped <#Constants constant>. A constant size argument must be
  -- non-negative and <#Representability representable> by a value of type
  -- @int@; if it is an untyped constant it is given type @int@. If both @n@
  -- and @m@ are provided and are constant, then @n@ must be no larger than
  -- @m@. If @n@ is negative or larger than @m@ at run time, a
  -- <#Run_time_panics run-time panic> occurs.
  --
  ...
  --
  -- Calling @make@ with a map type and size hint @n@ will create a map with
  -- initial space to hold @n@ map elements. The precise behavior is
  -- implementation-dependent.

  -- ** #Appending_and_copying_slices# Appending to and copying slices
  -- | The built-in functions @append@ and @copy@ assist in common slice
  -- operations. For both functions, the result is independent of whether the
  -- memory referenced by the arguments overlaps.
  --
  -- The <#Function_types variadic> function @append@ appends zero or more
  -- values @x@ to @s@ of type @S@, which must be a slice type, and returns
  -- the resulting slice, also of type @S@. The values @x@ are passed to a
  -- parameter of type @...T@ where @T@ is the <#Slice_types element type> of
  -- @S@ and the respective
  -- <#Passing_arguments_to_..._parameters parameter passing rules> apply. As
  -- a special case, @append@ also accepts a first argument assignable to
  ...
  -- arguments must have <#Type_identity identical> element type @T@ and must
  -- be <#Assignability assignable> to a slice of type @[]T@. The number of
  -- elements copied is the minimum of @len(src)@ and @len(dst)@. As a
  -- special case, @copy@ also accepts a destination argument assignable to
  -- type @[]byte@ with a source argument of a string type. This form copies
  -- the bytes from the string into the byte slice.
  --
  -- > copy(dst, src []T) int
  ...
  -- ** #Deletion_of_map_elements# Deletion of map elements
  -- | The built-in function @delete@ removes the element with key @k@ from a
  -- <#Map_types map> @m@. The type of @k@ must be
  -- <#Assignability assignable> to the key type of @m@.
  --
  -- > delete(m, k)  // remove element m[k] from map m
  --
  -- If the map @m@ is @nil@ or the element @m[k]@ does not exist, @delete@
  -- is a no-op.

  -- ** #Complex_numbers# Manipulating complex numbers
  -- | Three functions assemble and disassemble complex numbers. The built-in
  -- function @complex@ constructs a complex value from a floating-point real
  -- and imaginary part, while @real@ and @imag@ extract the real and
  -- imaginary parts of a complex value.
  --
  -- > complex(realPart, imaginaryPart floatT) complexT
  -- > real(complexT) floatT
  -- > imag(complexT) floatT
  --
  -- The type of the arguments and return value correspond. For @complex@,
  -- the two arguments must be of the same floating-point type and the return
  -- type is the complex type with the corresponding floating-point
  -- constituents: @complex64@ for @float32@ arguments, and @complex128@ for
  -- @float64@ arguments. If one of the arguments evaluates to an untyped
  -- constant, it is first <#Conversions converted> to the type of the other
  -- argument. If both arguments evaluate to untyped constants, they must be
  -- non-complex numbers or their imaginary parts must be zero, and the
  -- return value of the function is an untyped complex constant.
  --
  -- For @real@ and @imag@, the argument must be of complex type, and the
  -- return type is the corresponding floating-point type: @float32@ for a
  -- @complex64@ argument, and @float64@ for a @complex128@ argument. If the
  -- argument evaluates to an untyped constant, it must be a number, and the
  -- return value of the function is an untyped floating-point constant.
  --
  -- The @real@ and @imag@ functions together form the inverse of @complex@,
  -- so for a value @z@ of a complex type @Z@,
  ...
  -- ** #Handling_panics# Handling panics
  -- | Two built-in functions, @panic@ and @recover@, assist in reporting and
  -- handling <#Run_time_panics run-time panics> and program-defined error
  -- conditions.
  --
  ...
  -- <#Run_time_panics run-time panic> terminates the execution of @F@. Any
  -- functions <#Defer_statements deferred> by @F@ are then executed as
  -- usual. Next, any deferred functions run by @F\'s@ caller are run, and so
  -- on up to any deferred by the top-level function in the executing
  -- goroutine. At that point, the program is terminated and the error
  ...
  -- reaches @D@, the return value of @D@\'s call to @recover@ will be the
  -- value passed to the call of @panic@. If @D@ returns normally, without
  -- starting a new @panic@, the panicking sequence stops. In that case, the
  -- state of functions called between @G@ and the call to @panic@ is
  -- discarded, and normal execution resumes. Any functions deferred by @G@
  -- before @D@ are then run and @G@\'s execution terminates by returning to
  -- its caller.
  --
  ...
  --
  -- The @protect@ function in the example below invokes the function
  -- argument @g@ and protects callers from run-time panics raised by @g@.
  --
  -- > func protect(g func()) {
  -- >     defer func() {
  ...
  --
  -- Implementation restriction: @print@ and @println@ need not accept
  -- arbitrary argument types, but printing of boolean, numeric, and string
  -- <#Types types> must be supported.

  -- * #Packages# Packages
  -- | Go programs are constructed by linking together /packages/. A package in
  -- turn is constructed from one or more source files that together declare
  -- constants, types, variables and functions belonging to the package and
  -- which are accessible in all files of the same package. Those elements
  -- may be <#Exported_identifiers exported> and used in another package.

  -- ** #Source_file_organization# Source file organization
  ...
  --
  -- The PackageName must not be the <#Blank_identifier blank identifier>.
  --
  -- > package math
  --
  ...
  -- declaration depends on functionality of the /imported/ package
  -- (<#Program_initialization_and_execution §Program initialization and execution>)
  -- and enables access to <#Exported_identifiers exported> identifiers of
  -- that package. The import names an identifier (PackageName) to be used
  -- for access and an ImportPath that specifies the package to be imported.
  --
  ...
  -- <#Qualified_identifiers qualified identifiers> to access exported
  -- identifiers of the package within the importing source file. It is
  -- declared in the <#Blocks file block>. If the PackageName is omitted, it
  -- defaults to the identifier specified in the
  -- <#Package_clause package clause> of the imported package. If an explicit
  -- period (@.@) appears instead of a name, all the package\'s exported
  -- identifiers declared in that package\'s <#Blocks package block> will be
  ...
  -- <https://www.unicode.org/versions/Unicode6.3.0/ Unicode\'s> L, M, N, P,
  -- and S general categories (the Graphic characters without spaces) and may
  -- also exclude the characters @!\"#$%&\'()*,:;\<=>?[\\]^\`{|}@ and the
  -- Unicode replacement character U+FFFD.
  --
  -- Assume we have compiled a package containing the package clause
  -- @package math@, which exports function @Sin@, and installed the compiled
  -- package in the file identified by @\"lib\/math\"@. This table
  -- illustrates how @Sin@ is accessed in files that import the package after
  -- the various types of import declaration.
  --
  ...
  -- itself, directly or indirectly, or to directly import a package without
  -- referring to any of its exported identifiers. To import a package solely
  -- for its side-effects (initialization), use the <#Blank_identifier blank>
  ...
  -- ** #The_zero_value# The zero value
  -- | When storage is allocated for a <#Variables variable>, either through a
  ...
  -- value/ for its type: @false@ for booleans, @0@ for numeric types, @\"\"@
  -- for strings, and @nil@ for pointers, functions, interfaces, slices,
  -- channels, and maps. This initialization is done recursively, so for
  -- instance each element of an array of structs will have its fields zeroed
  ...
  -- /declaration order/ but after any of the variables they /depend/ on.
  --
  ...
  -- <#Variable_declarations initialization expression> or its initialization
  -- expression has no dependencies on uninitialized variables.
  -- Initialization proceeds by repeatedly initializing the next
  -- package-level variable that is earliest in declaration order and ready
  -- for initialization, until there are no variables ready for
  -- initialization.
  --
  -- If any variables are still uninitialized when this process ends, those
  -- variables are part of one or more initialization cycles, and the program
  -- is not valid.
  --
  ...
  -- compiler: Variables declared in the first file are declared before any
  -- of the variables declared in the second file, and so on.
  --
  -- Dependency analysis does not rely on the actual values of the variables,
  -- only on lexical /references/ to them in the source, analyzed
  -- transitively. For instance, if a variable @x@\'s initialization
  -- expression refers to a function whose body refers to variable @y@ then
  -- @x@ depends on @y@. Specifically:
  --
  -- -   A reference to a variable or function is an identifier denoting that
  --     variable or function.
  -- -   A reference to a method @m@ is a <#Method_values method value> or
  --     <#Method_expressions method expression> of the form @t.m@, where the
  --     (static) type of @t@ is not an interface type, and the method @m@ is
  --     in the <#Method_sets method set> of @t@. It is immaterial whether
  --     the resulting function value @t.m@ is invoked.
  -- -   A variable, function, or method @x@ depends on a variable @y@ if
  --     @x@\'s initialization expression or body (for functions and methods)
  --     contains a reference to @y@ or to a function or method that depends
  --     on @y@.
  --
  -- Dependency analysis is performed per package; only references referring
  -- to variables, functions, and methods declared in the current package are
  -- considered.
  --
  -- For example, given the declarations
  --
  ...
  -- source file. In the package block, the @init@ identifier can be used
  -- only to declare @init@ functions, yet the identifier itself is not
  -- <#Declarations_and_scope declared>. Thus @init@ functions cannot be
  -- referred to from anywhere in a program.
  --
  -- A package with no imports is initialized by assigning initial values to
  -- all its package-level variables followed by calling all @init@ functions
  -- in the order they appear in the source, possibly in multiple files, as
  -- presented to the compiler. If a package has imports, the imported
  -- packages are initialized before initializing the package itself. If
  -- multiple packages import a package, the imported package will be
  -- initialized only once. The importing of packages, by construction,
  -- guarantees that there can be no cyclic initialization dependencies.
  --
  ...
  -- initialization always sequences the @init@ functions: it will not invoke
  -- the next one until the previous one has returned.
  --
  ...
  -- * #Run_time_panics# Run-time panics
  -- | Execution errors such as attempting to index an array out of bounds
  -- trigger a /run-time panic/ equivalent to a call of the built-in function
  -- <#Handling_panics panic> with a value of the implementation-defined
  -- interface type @runtime.Error@. That type satisfies the predeclared
  -- interface type <#Errors error>. The exact error values that represent
  ...
  -- * #System_considerations# System considerations

  -- ** #Package_unsafe# Package @unsafe@
  -- | The built-in package @unsafe@, known to the compiler and accessible
  -- through the <#Import_declarations import path> @\"unsafe\"@, provides
  ...
  -- type safety and may not be portable. The package provides the following
  -- interface:
  --
  -- > package unsafe
  -- >
  ...
  --
  -- A @Pointer@ is a <#Pointer_types pointer type> but a @Pointer@ value may
  -- not be <#Address_operators dereferenced>. Any pointer or value of
  -- <#Types underlying type> @uintptr@ can be converted to a type of
  -- underlying type @Pointer@ and vice versa. The effect of converting
  -- between @Pointer@ and @uintptr@ is implementation-defined.
  --
  -- > var f float64
  ...
  -- > var p ptr = nil
  --
  -- The functions @Alignof@ and @Sizeof@ take an expression @x@ of any type
  -- and return the alignment or size, respectively, of a hypothetical
  -- variable @v@ as if @v@ was declared via @var v = x@.
  --
  -- The function @Offsetof@ takes a (possibly parenthesized)
  -- <#Selectors selector> @s.f@, denoting a field @f@ of the struct denoted
  -- by @s@ or @*s@, and returns the field offset in bytes relative to the
  -- struct\'s address. If @f@ is an <#Struct_types embedded field>, it must
  -- be reachable without pointer indirections through fields of the struct.
  ...
  -- variable\'s type\'s /alignment/. The function @Alignof@ takes an
  -- expression denoting a variable of any type and returns the alignment of
  -- the (type of the) variable in bytes. For a variable @x@:
  --
  -- > uintptr(unsafe.Pointer(&x)) % unsafe.Alignof(x) == 0
  --
  -- Calls to @Alignof@, @Offsetof@, and @Sizeof@ are compile-time constant
  -- expressions of type @uintptr@.

  -- ** #Size_and_alignment_guarantees# Size and alignment guarantees
  -- | For the <#Numeric_types numeric types>, the following sizes are
  -- guaranteed:
  --
  -- > type                                 size in bytes
  -- >
  ...
  -- 2.  For a variable @x@ of struct type: @unsafe.Alignof(x)@ is the
  --     largest of all the values @unsafe.Alignof(x.f)@ for each field @f@
  --     of @x@, but at least 1.
